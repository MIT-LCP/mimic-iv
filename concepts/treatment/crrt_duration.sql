...
(
  ...
  WINDOW w AS 
  (
    partition by stay_id, case when RRT=1 or RRT_end=1 then 1 else 0 end
    order by charttime
  )
)
, vd1 as
...
select stay_id
  ...
  -- add durations
from vd2
group by stay_id, num
having min(charttime) != max(charttime)
)
select stay_id
  , ROW_NUMBER() over (partition by stay_id order by starttime) as num
  , starttime, endtime, duration_hours
from fin
order by stay_id, num
